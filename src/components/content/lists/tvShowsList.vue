...
const props = defineProps<{
  tvShows: CollectionEntry<"tv-shows">[];
  maxPage: number;
}>();
...
const displayedTvShows = ref<CollectionEntry<"tv-shows">[]>([]);
const pageNum = ref(1);
 ...
 */
const calculatePage = (
  caculateResults: CollectionEntry<"tv-shows">[],
  index: number
) => {
...
function onIntersectionObserver([{ isIntersecting }]) {
  if (isIntersecting) {
    ...
    if (updatedPageNum <= props.maxPage) {
      displayedTvShows.value = displayedTvShows.value.concat(
        calculatePage(props.tvShows, updatedPageNum)
      );
...
const reorderDisplayedTvShows = (order: string) => {
  const updateDisplayedTvShows = displayedTvShows.value.sort((a, b) => {
    if (order === "name-asc") {
  ...
  displayedTvShows.value = updateDisplayedTvShows;
};
 ...
 */
onMounted(() => {
  displayedTvShows.value = calculatePage(props.tvShows, 1);
});

watch(selectedOrder, async (newOrder, _) => {
  reorderDisplayedTvShows(newOrder);
});
...
<template>
  <div>
    ...
    >
      <a
        v-for="item in displayedTvShows"
        :href="`/tv-shows/${item.slug}`"
